...
export default function HorizontalBar({     //Renders a horizontal bar chart
  ...
  useEffect(() => {
    ...
    const y = d3//
      .scaleBand<string>()//Given a list of category names, tell me where each bar should sit and how wide it should be
      ...
      .padding(0.6);//Adds padding between bars o: bars touch 1: almost no bars
    ...
    g.append("g")//Appends an SVG group element for the x-axis
      ...
      .call(d3.axisBottom(x).ticks(10))//Creates the bottom axis using the x scale with 5 ticks
      .call((sel) => sel.selectAll("text").attr("fill", "rgba(190, 29, 156, 0.72)").attr("font-size", 10))//Styles the tick labels
      .call((sel) => sel.selectAll("path,line").attr("stroke", "rgba(255,255,255,.12)").attr("stroke-width", 3)); // Styles the axis lines
